...
-   Base → Region → VPC → Subnet → EC2 → Network Interface まで配置された状態を前提として、
    Load Balancer の設定を行います。
...
-   外部インターネットから発生したトラフィックが VPC 内部のリソースへ到達できるようにするため、
    VPC のインターネット出入口として機能する **Internet Gateway** を配置します。
...
-   外部からのリクエストを単一のエントリーポイントで受信し、
    複数の EC2 インスタンスへトラフィックを分散処理するため、
    **Load Balancer** を配置します。
-   Load Balancer を、異なるアベイラビリティゾーンに設定された
    各 Subnet に接続します。
...
-   Load Balancer が実際にトラフィックを転送する対象リソースの集合を定義するため、
    **Load Balancer Target Group** を配置し、
    **[Load Balancer -> Load Balancer Target Group]** の方向に線を接続します。

-   Load Balancer Target Group を、各 Subnet に配置された EC2 インスタンスへ接続し、
    Load Balancer がリクエストを分散できるように構成します。
...
-   Load Balancer の **[Listener - Linked Target Group Name]** オプションで、
    接続した **Target Group** の名前を選択します。
...
-   許可されたポートおよびプロトコルのみが Load Balancer にアクセスできるようにするため、
    Security Group を配置し、
    **[Security Group -> Load Balancer]** の方向に線を接続します。
