---
title: CustomChart
sidebar_position: 0
tags: [Kubernetes, 리소스, Helm, CustomChart]
---

<hr />
<br />

## Resource Icon

<img
    src={
        "https://cloudstudioblobprod.blob.core.windows.net/cloudstudio/kubernetesImg/helm-custom_chart.webp"
    }
/>

## Resource Overview

A resource that allows deploying a Helm chart by configuring it with user-defined values.

## Associated Resources

### Parent Resource

-   [**Namespace**](https://docs.cstudio.app/docs/resource/kubernetes/Core/Namespace)

## Resource Setting Values

#### Basic Settings

-   `namespace` : Namespace where the custom Helm Chart resource will be deployed
-   `repository` : Repository URL where the Helm Chart to customize is stored
-   `chart` : Name of the Helm Chart to customize
-   `version` : Version of the Helm Chart to customize
-   `set` : Key-value settings used to dynamically override values of the base Helm Chart

---

## References

-   [Helm Provider(Terraform)](https://registry.terraform.io/providers/hashicorp/helm/latest/docs)
