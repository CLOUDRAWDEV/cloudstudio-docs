---
title: NginxIngress
sidebar_position: 6
tags: [Kubernetes, 리소스, Helm, NginxIngress]
---

<hr />
<br />

## Resource Icon

<img
    src={
        "https://cloudstudioblobprod.blob.core.windows.net/cloudstudio/kubernetesImg/helm-nginx_ingress.webp"
    }
/>

## Resource Overview

A Helm chart for installing the Nginx Ingress Controller.

## Associated Resources

### Parent Resource

-   [**Namespace**](https://docs.cstudio.app/docs/resource/kubernetes/Core/Namespace)

### Connected Resources

-   [**Ingress**](https://docs.cstudio.app/docs/resource/kubernetes/Networking/Ingress)

## Resource Setting Values

#### Basic Settings

-   `namespace` : Namespace where the Nginx Ingress Helm Chart resource will be deployed
-   `version` : Version of the Nginx Ingress Helm Chart to use
-   `replica_count` : Number of replicas for the Ingress Controller Deployment
-   `external_traffic_policy` : External traffic handling policy - `Cluster `, `Local`
-   `ingress_class_name` : Name of the Ingress Class managed by this Ingress Controller
-   `set` : Key-value settings used to dynamically override values of the base Nginx Ingress Controller Helm Chart

---

## References

-   [Helm install(NginxIngress)](https://docs.nginx.com/nginx-ingress-controller/installation/installing-nic/installation-with-helm/)
-   [Helm Provider(Terraform)](https://registry.terraform.io/providers/hashicorp/helm/latest/docs)
