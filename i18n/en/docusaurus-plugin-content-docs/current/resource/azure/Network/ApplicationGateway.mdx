...
Application Gateway is an Azure managed load balancer that performs **Layer 7 (application layer) load balancing and routing** at the front end of web applications based on HTTP/HTTPS traffic.  
It provides both security and scalability for web services through features such as path-based routing, SSL termination, and integration with WAF.
...
-   `instance_count` : Number of instances (for V1 SKU)
...
-   `frontend_ip_address_type` : Frontend IP mode - `Public`, `Private`, `Both`
...
-   `backend_pool.target.linked_target_name` : Name of the Backend resource to connect
...
-   `backend_http_settings.protocol` : Protocol used when forwarding to the Backend
...
-   `request_routing_rule.priority` : Priority of the routing rule
...
-   [Integrating WAF with Application Gateway (Microsoft)](https://learn.microsoft.com/ko-kr/azure/web-application-firewall/ag/ag-overview)
