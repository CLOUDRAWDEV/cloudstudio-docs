...
FirewallPolicyRuleCollectionGroup is a resource that **groups multiple rule collections within an Azure Firewall Policy for centralized management**.  
Application, NAT, and Network Rule Collections can be organized into a single group, and the processing order within the policy can be controlled based on group priority.
