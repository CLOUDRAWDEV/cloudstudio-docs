...
-   `endpoint_names` : List of endpoint names to be used

### Origin Group Settings (`origin_group`)

-   `restore_traffic_time_in_minutes` : Wait time (minutes) before traffic failover when an endpoint becomes unavailable or when a new endpoint is added – `0` ~ `50`

-   `session_affinity_enabled` : Whether to enable session affinity – `true`, `false`

-   `health_probe_path` : Relative path used to check the health of origins

-   `health_probe_protocol` : Protocol used for health probes – `Http`, `Https`

-   `health_probe_method` : Request method for health probes – `HEAD`, `GET`
...
-   `endpoint_name` : Name of the Front Door endpoint to which the route is applied

-   `origin_group_name` : Name of the origin group to which traffic is forwarded
