...
Azure Container Apps is a managed execution environment that allows container-based applications to run in a serverless model.  
Based on a connected Container Apps Environment, you can deploy and operate applications by configuring revision management, init containers, storage volume mounts, and ingress (external access) settings.
...
-   `min_replicas` : Minimum number of container instances to keep running at all times - `1` ~ `300`
...
-   `container.liveness_probe.host` : Host name to send probe requests to

-   `container.liveness_probe.initial_delay` : Delay (seconds) before the first probe after container start - `0` ~ `60`

-   `container.liveness_probe.interval_seconds` : Interval (seconds) between probe executions - `1` ~ `240`

-   `container.liveness_probe.path` : Path to call for HTTP probe requests

-   `container.liveness_probe.port` : Container port number to receive probe requests - `1` ~ `65535`

-   `container.liveness_probe.timeout` : Maximum wait time for a response (seconds) - `1` ~ `240`
...
-   `ingress.enable_insecure_connections` : Whether to allow unsecured HTTP connections - `true`, `false`

-   `ingress.enable_external_connections` : Whether to allow access from outside the container app - `true`, `false`
...
-   `ingress.target_port` : Port number that the container listens on internally
