...
By configuring basic cluster settings, authentication/authorization (RBAC), network policies, node pool configuration, and API server access control, container-based workloads can be efficiently operated.
...
-   `sku_tier` : Pricing tier for using the Kubernetes cluster – `Free`, `Standard`

-   `enable_public_network_access` : Whether to allow access from the public network – `true`, `false`

-   `api_server_authorized_ip_ranges` : List of authorized IP address ranges to secure access to the API server

-   `authentication_authorization` : Authorization method to restrict access to cluster resources – `local_accounts`, `azure_rbac`, `kubernetes_rbac`
...
-   `network_policy` : Network policy to restrict access to specific Pods – `calico`, `azure`, `none`
...
-   `pod_cidr` : CIDR used for assigning Pod IP addresses
...
-   `default_node_pool.vm_size` : Virtual machine size used to form nodes in the Node Pool

-   `default_node_pool.os_disk_size_gb` : OS disk size of the virtual machine (GB) – `30` ~ `2048`

-   `default_node_pool.max_pods` : Maximum number of Pods that can run on a node – `10` ~ `250`

-   `default_node_pool.enable_node_public_ip` : Whether to enable a public IP address per node – `true`, `false`

-   `default_node_pool.public_ip_prefix_name` : Resource ID of the public IP address prefix for nodes in this Node Pool

-   `default_node_pool.temporary_name_for_rotation` : Temporary Node Pool name that must be specified when certain values are changed
...
-   `cluster_node_pool.vm_size` : Virtual machine size used to form nodes in the Node Pool

-   `cluster_node_pool.os_disk_size_gb` : OS disk size of the virtual machine (GB) – `30` ~ `2048`

-   `cluster_node_pool.max_pods` : Maximum number of Pods that can run on a node – `10` ~ `250`

-   `cluster_node_pool.enable_node_public_ip` : Whether to enable a public IP address per node – `true`, `false`

-   `cluster_node_pool.public_ip_prefix_name` : Resource ID of the public IP address prefix for nodes in this Node Pool
