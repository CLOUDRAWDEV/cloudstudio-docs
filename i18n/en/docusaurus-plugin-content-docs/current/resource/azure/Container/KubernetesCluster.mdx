...
You can operate container-based workloads by configuring cluster basics, authentication and authorization (RBAC), network policies, node pool settings, and API server access control.
...
-   `dns_prefix` : DNS prefix used with the hosted Kubernetes API server FQDN
...
-   `api_server_authorized_ip_ranges` : List of authorized IP address ranges to protect access to the API server
...
-   `admin_group_object_ids` : List of object IDs for Azure Active Directory groups that have administrator roles on the cluster
...
-   `network_plugin` : Networking model used for the cluster - `kubenet`, `azure`

-   `included_subnet_name` : Name of the Subnet where the Kubernetes Cluster will be deployed

-   `pod_cidr` : CIDR range used for Pod IP addresses

-   `service_cidr` : CIDR range used to allocate service cluster IPs
...
-   `default_node_pool.vm_size` : Virtual machine size used for nodes in the Node Pool
...
-   `default_node_pool.temporary_name_for_rotation` : Temporary Node Pool name required when changing certain values
...
-   `cluster_node_pool.vm_size` : Virtual machine size used for nodes in the Node Pool
