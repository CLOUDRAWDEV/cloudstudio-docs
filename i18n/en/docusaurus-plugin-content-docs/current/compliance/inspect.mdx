...
sidebar_position: 2
tags: [Inspection, Console Inspection]
---
...
## Console Inspection Permission Inquiry

To inquire about console inspection permissions, click the `ⓘ Button`.

<img width="400" src={compliance22} alt="조회하기 이미지" />
<p></p>
<img width="400" src={compliance23} alt="조회하기 이미지" />
<p></p>
...
1. Click the `Inspection > Inspection Management > Console Inspection` tab to access console inspection.
2. Select the `group you want to inspect` from the group list.

:::info

Console inspections are performed based on the selected group. Please `create a group first` before starting console inspections.
[How to Create a Group](/docs/compliance/group)

:::

3. Click the `Console Inspection tab`.
...
In the inspection results, `Y` means compliant, `N` means vulnerable, `N/A` means not inspectable, and `P` means average.
...
2. After selecting Compliance, click the Generate button.

    <img width="400" src={compliance13} alt="이미지" />

3. Open the Excel file to check the inspection details.
...
2. In the inspection history, you can check the `Inspection Title, Inspection Execution Date & Time`.
