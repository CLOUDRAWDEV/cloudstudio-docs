...
A Kubernetes service, an open-source platform that automates application containerization, deployment, and management. It improves work efficiency by automating tasks required to use containers, such as container deployment, management, and scaling.
...
-   `subnet_names` : List of subnet names that 구성e the cluster, up to 3
-   `lb_private_subnet_name` : Subnet name where the load balancer integrated with the cluster is located
-   `lb_public_subnet_name` : Subnet name where the load balancer integrated with the cluster is located
...
-   `node_pool.taint.taint_key` : Key to set for node taint
-   `node_pool.taint.taint_value` : Value to set for node taint
...
-   [Ncloud Kubernetes Service Cluster(Terraform)](https://registry.terraform.io/providers/NaverCloudPlatform/ncloud/latest/docs/resources/nks_cluster)
