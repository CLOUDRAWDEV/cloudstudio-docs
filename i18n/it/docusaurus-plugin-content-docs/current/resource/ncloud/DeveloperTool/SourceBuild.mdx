...
SourceBuild is a service that makes it easy to build source code developed in various languages.  
It creates independent build servers in real time and can process multiple build execution requests simultaneously.
...
-   `env.env_compute_spec` : Build compute environment specification - `2cpu-4memory`, `4cpu-8memory`, `8cpu-16memory`
...
-   `enable_file_safer` : Whether to perform security checks using File Safer - `true`, `false`
...
-   `upload.upload_build_results` : Whether to upload the build environment image - `true`, `false`
-   `upload.container_registry_name_to_upload_build_results` : Name of the container registry for image upload
