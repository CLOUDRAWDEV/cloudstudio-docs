...
<br />
11. 왼쪽 사이드 바에서 `인증서 및 암호`를 클릭합니다. 12. `새 클라이언트 암호`를
클릭합니다. 13. Key에 대한 간단한 설명을 입력한 후 만료시간을 선택합니다. 입력이
완료되면 `추가`를 클릭합니다. 14. 생성된 값을 복사하여 클라우드 스튜디오 Service
Principal Key에 붙여넣기 합니다.
